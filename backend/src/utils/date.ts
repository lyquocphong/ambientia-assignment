...
export const generateMomentFromIsoString = (isoString: string) => {
  const parsedDate = moment(isoString, moment.ISO_8601, true);
  return parsedDate.isValid() ? parsedDate : null;
};

export const isValidRange = (from: string, to: string) => {
  const fromMoment = generateMomentFromIsoString(from);
  const toMoment = generateMomentFromIsoString(to);

  return (
    fromMoment !== null && toMoment !== null && toMoment.isAfter(fromMoment)
  );
};
...
export const isTimeInRange = (
  ...
  if (
    !isValidTimeString(timeToCheck) ||
    !isValidTimeString(scheduleFrom) ||
    !isValidTimeString(scheduleTo)
  ) {
...
export const formatDate = (moment: moment.Moment, format?: string): string => {
